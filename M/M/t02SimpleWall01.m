...
% ************************************
% Step response
dh = 1.98;    % if dh = 1.98, stable; if dh = 1.97, instable
dt = 3600/dh; % simulation step 1h/dt = 3600s / dt]
n = dh*24*30; % no of time samples for 30 days
n = floor(n);
...
%Stability
lambda = eig(A);
min(lambda)*dt; %needs to be in [-2 0]
...
disp('Mean temperatures In Out')
disp([mean(th(7,:)) mean(Temp)])