...
%****************************
% Parameters
Kp = 1e3;               % P-controller gain: large for precision
...
% *********************************************************************
[Ac,Bc,Cc,Dc] = fTC2SS(A,G,b,C,f,y);    % model with controller

G4 = diag([Gv 1e-1]);   % no controller: practically Kp = 0 
TCd{4} = {A4,G4,b4,C4,f4,y4};
[TCa, Idx] = fTCAssAll(TCd, AssX);
A = TCa{1}; G = TCa{2}; b = TCa{3}; C = TCa{4}; f = TCa{5}; y = TCa{6};
[Af,Bf,Cf,Df] = fTC2SS(A,G,b,C,f,y);    % model free-running

% Maximum time-step
dtmax = min([-2./eig(Ac) -2./eig(Af)])% [s]
...
n = size(Time,1);
nth = size(Ac,1);       % no of state variables
...
th = zeros(size(Ac,2),n);
y = zeros(1,n); y(1) = TintSP(1);
qHVAC = zeros(1,n);
for k = 1:n-1
    if y(k) < TintSP(k) || y(k) > 4 + TintSP(k)
        th(:,k+1) = (eye(nth) + dt*Ac)*th(:,k) + dt*Bc*u(:,k);
        y(:,k+1) = Cc*th(:,k+1) + Dc*u(:,k+1);
        qHVAC(:,k+1) = Kp*(TintSP(k+1) - y(k+1));
    else
        th(:,k+1) = (eye(nth) + dt*Af)*th(:,k) + dt*Bf*u(:,k);
        y(:,k+1) = Cf*th(:,k+1) + Df*u(:,k+1);
        qHVAC(:,k+1) = 1e-1*(TintSP(k+1) - y(k+1));
    end
end
subplot(3,1,1)
plot(Time/3600,y,Time/3600,TintSP, Time/3600,Temp)
xlabel('Time [h]'),ylabel('T [C]')
title('Simulation for weather')

subplot(3,1,2)
plot(Time/3600,qHVAC)
xlabel('Time [h]'),ylabel('Q_h [W]')
title('Q_h')
axis([0 inf -10000 inf])
